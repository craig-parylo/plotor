...
+=====================+=====================+=====================+
| The outcome         | `plotor` is         | ✅                  |
| variable is         | designed to work    |                     |
| **binary**          | with an outcome     | Introduced in PR    |
|                     | variable that has   | [42](https          |
|                     | only two possible   | ://%20githu%20b.com |
|                     | values, i.e.        | /cr%20aig-paryl%20o |
|                     | outcome is          | /pl%20otor/pull/42) |
|                     | **binary**.         |                     |
+---------------------+---------------------+---------------------+
| The predictor       | Predictor variables | ✅                  |
| variables should    | which have high     |                     |
| **not be highly     | levels of           | Introduced in PR    |
| correlated** with   | correlation with    | [43](https          |
| each other          | each other is known | ://%20githu%20b.com |
|                     | as                  | /cr%20aig-paryl%20o |
|                     | **m                 | /pl%20otor/pull/43) |
|                     | ulticollinearity**. |                     |
|                     |                     |                     |
|                     | Where this is the   |                     |
...
|                     | results.            |                     |
+---------------------+---------------------+---------------------+
| The outcome is      | In logistic         | ✅                  |
| **not separated**   | regression,         |                     |
| by predictors       | **separation**      | Introduced in PR    |
|                     | occurs when a       | [47](https          |
|                     | predictor variable  | ://%20githu%20b.com |
|                     | (or a combination   | /cr%20aig-paryl%20o |
|                     | of predictor        | /pl%20otor/pull/47) |
|                     | variables)          |                     |
|                     | perfectly predicts  |                     |
|                     | the outcome         |                     |
|                     | variable.           |                     |
...
|                     | coefficients.       |                     |
+---------------------+---------------------+---------------------+
| The sample size is  | The sample size     | ✅                  |
| large enough        | should be large     |                     |
|                     | enough to provide   | Introduced in PR    |
|                     | reliable estimates  |                     |
|                     | of the odds ratio.  |                     |
|                     | A general rule of   |                     |
|                     | thumb is to have at |                     |
...
|                     | predictor variable. |                     |
+---------------------+---------------------+---------------------+
| The observations    | Each observation    | In development      |
| are independent     | should be           |                     |
|                     | independent of the  |                     |
|                     | others. This means  |                     |
|                     | that the outcome    |                     |
|                     | for one observation |                     |
...
|                     | observation.        |                     |
+---------------------+---------------------+---------------------+
| There are no        | The data should not | In development      |
| extreme outlier     | contain outliers or |                     |
| values              | influential         |                     |
|                     | observations that   |                     |
|                     | can significantly   |                     |
|                     | affect the          |                     |
...
|                     | odds ratio.         |                     |
+---------------------+---------------------+---------------------+
| There is a linear   | The relationship    | In development      |
| relationship        | between the         |                     |
| between the         | predictor variables |                     |
| predictors and the  | and the log odds of |                     |
| logit               | the outcome should  |                     |
|                     | be linear. This can |                     |
|                     | be checked using    |                     |
|                     | diagnostic plots,   |                     |
|                     | such as the logit   |                     |
