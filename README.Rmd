...
| **binary**    | two possible values, i.e. outcome  | Introduced in |
|               | is **binary**.                     | PR [42](#42)  |
+---------------+------------------------------------+---------------+
| The predictor | Predictor variables which have     | ✅            |
| variables     | high levels of correlation with    |               |
| should **not  | each other is known as             | Introduced in |
| be highly     | **multicollinearity**.             | PR [43](#43)  |
| correlated**  |                                    |               |
| with each     | Where this is the case the odds    |               |
| other         | ratio estimates are likely to be   |               |
|               | unstable, confidence intervals are |               |
...
| separated**   | predictor variable (or a           | Introduced in |
| by predictors | combination of predictor           | PR [47](#47)  |
...
| size is large | enough to provide reliable         | development   |
| enough        | estimates of the odds ratio. A     |               |
|               | general rule of thumb is to have   |               |
|               | at least 10 events (or outcomes of |               |
|               | interest) per predictor variable.  |               |
+---------------+------------------------------------+---------------+
...
| independent   | observation should not be          |               |
|               | influenced by the outcome of       |               |
|               | another observation.               |               |
+---------------+------------------------------------+---------------+
| There are no  | The data should not contain        | In            |
| extreme       | outliers or influential            | development   |
| outlier       | observations that can              |               |
| values        | significantly affect the estimates |               |
|               | of the odds ratio.                 |               |
+---------------+------------------------------------+---------------+
| There is a    | The relationship between the       | In            |
| linear        | predictor variables and the log    | development   |
| relationship  | odds of the outcome should be      |               |
| between the   | linear. This can be checked using  |               |
| predictors    | diagnostic plots, such as the      |               |
| and the logit | logit plot.                        |               |
+---------------+------------------------------------+---------------+
