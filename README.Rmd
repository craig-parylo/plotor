...
+===================+===================================+==========+
| The outcome       | `plotor` is designed to work with | ✅       |
| variable is       | an outcome variable that has only |          |
| **binary**        | two possible values, i.e. outcome |          |
|                   | is **binary**.                    |          |
+-------------------+-----------------------------------+----------+
| The predictor     | Predictor variables which have    | ✅       |
| variables should  | high levels of correlation with   |          |
| **not be highly   | each other is known as            |          |
| correlated** with | **multicollinearity**.            |          |
| each other        |                                   |          |
|                   | Where this is the case the odds   |          |
|                   | ratio estimates are likely to be  |          |
|                   | unstable, confidence intervals    |          |
|                   | are likely to be much larger,     |          |
|                   | both of which make it difficult   |          |
|                   | to interpret the results.         |          |
+-------------------+-----------------------------------+----------+
| The outcome is    | In logistic regression,           | ✅       |
| **not separated** | **separation** occurs when a      |          |
| by predictors     | predictor variable (or a          |          |
|                   | combination of predictor          |          |
|                   | variables) perfectly predicts the |          |
|                   | outcome variable.                 |          |
|                   |                                   |          |
|                   | Separation results in infinite or |          |
|                   | extremely large odds ratios and   |          |
|                   | possibly issues with              |          |
|                   | non-convergence of the logistic   |          |
|                   | regression model, making it       |          |
|                   | difficult for the model to        |          |
|                   | estimate the coefficients.        |          |
+-------------------+-----------------------------------+----------+
| The sample size   | The sample size should be large   | ✅       |
| is large enough   | enough to provide reliable        |          |
|                   | estimates of the odds ratio. A    |          |
|                   | general rule of thumb is to have  |          |
|                   | at least 10 events (or outcomes   |          |
|                   | of interest) per predictor        |          |
|                   | variable.                         |          |
+-------------------+-----------------------------------+----------+
| The observations  | Each observation should be        | In       |
| are independent   | independent of the others. This   | dev      |
|                   | means that the outcome for one    | elopment |
|                   | observation should not be         |          |
|                   | influenced by the outcome of      |          |
|                   | another observation.              |          |
+-------------------+-----------------------------------+----------+
| There are no      | The data should not contain       | In       |
| extreme outlier   | outliers or influential           | dev      |
| values            | observations that can             | elopment |
|                   | significantly affect the          |          |
|                   | estimates of the odds ratio.      |          |
+-------------------+-----------------------------------+----------+
| There is a linear | The relationship between the      | In       |
| relationship      | predictor variables and the log   | dev      |
| between the       | odds of the outcome should be     | elopment |
| predictors and    | linear. This can be checked using |          |
| the logit         | diagnostic plots, such as the     |          |
|                   | logit plot.                       |          |
+-------------------+-----------------------------------+----------+
